...
/**
 * @file
 * @brief Host regressions for parsing, safety, cancellation and loop budgets.
 */
...
void test_completion_wraparound_and_no_catchup_burst() {
  ...
  fakeMillis = start;
  f.experiment.update(start);
  fakeMillis = start + 4000U;
...
void test_runtime_write_failure_faults_and_cancels() {
  ...
  TEST_ASSERT_FALSE(f.experiment.active());
  TEST_ASSERT_EQUAL(0, f.experiment.remainingMs(0));
  TEST_ASSERT_EQUAL(1, f.experiment.stats().faulted);
  TEST_ASSERT_EQUAL(0, f.can.stats().txFrames);
  TEST_ASSERT_EQUAL(-7, f.can.stats().lastWriteResult);
  f.command("reset\narm\nstart\ndisarm\n");
...
void test_async_controller_error_and_external_fault() {
  ...
  g.command("arm\nstart\n");
  g.safety.fault();
  TEST_ASSERT_FALSE(g.experiment.active());
  g.experiment.update(0);
  TEST_ASSERT_EQUAL(1, g.experiment.stats().faulted);
  TEST_ASSERT_EQUAL(0, CAN.writeCalls);
}
...
void test_bitrate_validation_and_initialization_failure() {
  ...
  s.begin();
  CanDriver driver(s);
  TEST_ASSERT_FALSE(driver.begin(123456));
  TEST_ASSERT_EQUAL(0, CAN.beginCalls);
  TEST_ASSERT_EQUAL(CanStatus::Offline, driver.status());
  TEST_ASSERT_EQUAL(0, driver.bitrate());
  CAN.beginResult = false;
  TEST_ASSERT_FALSE(driver.begin(500000));
  TEST_ASSERT_EQUAL(SafetyState::Fault, s.state());
  TEST_ASSERT_EQUAL(CanStatus::Error, driver.status());
  for (uint32_t bitrate : {125000U, 250000U, 500000U}) {
    CAN = FakeCAN{};
    SafetyManager other;
    other.begin();
    CanDriver can(other);
    TEST_ASSERT_TRUE(can.begin(bitrate));
    TEST_ASSERT_EQUAL(bitrate, CAN.bitrate);
    TEST_ASSERT_FALSE(can.begin(123456));
    TEST_ASSERT_EQUAL(bitrate, can.bitrate());
  }
}
...
void test_frame_formats_and_write_contract() {
  ...
  TEST_ASSERT_EQUAL(0, CAN.writeCalls);
  frame.length = 8;
  TEST_ASSERT_TRUE(f.can.send(frame));
  TEST_ASSERT_EQUAL(1, CAN.writeCalls);
  uint8_t data[8] = {1, 2, 3};
  CAN.rx.emplace_back(CanExtendedId(0x123), 3, data);
  TEST_ASSERT_TRUE(f.can.receive(frame));
  TEST_ASSERT_TRUE(frame.extended);
  TEST_ASSERT_EQUAL(0x123, frame.id);
...
void test_reset_counters_and_status() {
  ...
  TEST_ASSERT_FALSE(f.experiment.active());
  TEST_ASSERT_EQUAL(0, f.can.stats().txFrames);
  TEST_ASSERT_EQUAL(0, f.can.stats().rxFrames);
  TEST_ASSERT_EQUAL(0, f.can.stats().txErrors);
  TEST_ASSERT_EQUAL(0, f.experiment.stats().started);
  TEST_ASSERT_EQUAL(0, f.experiment.stats().stopped);
...
void test_logging_budget_backpressure_and_whole_line_drops() {
  ...
  out.drain(sink, 32);
  TEST_ASSERT_EQUAL(32, sink.output.size());
  sink.writable = false;
  out.drain(sink, 32);
  TEST_ASSERT_EQUAL(32, sink.output.size());
  sink.writable = true;
  ...
  out.resetStats();
  TEST_ASSERT_EQUAL(0, out.droppedLines());
}
...
void test_actual_loop_bounds_and_stop_with_rx_backlog() {
  ...
  uint8_t data[8] = {};
  for (int i = 0; i < 100; ++i) {
    CAN.rx.emplace_back(CanStandardId(0x123), 8, data);
  }
  Serial.feed("stop\n");
  auto writes = CAN.writeCalls;
  auto bytes = Serial.output.size();
  fakeMillis += 2000;
  loop();
  TEST_ASSERT_FALSE(experiment.active());
  TEST_ASSERT_EQUAL(writes, CAN.writeCalls);
